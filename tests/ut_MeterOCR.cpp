...
{
  std::list<Option> options;
  options.push_back(Option("file", (char*)"tests/meterOCR/img2_blue.png"));
  options.push_back(Option("generate_debug_image", false));
  options.push_back(Option("impulses", 10000));
  options.push_back(Option("rotate", -2.0)); // rotate by -2deg (counterclockwise)// 661, 539, 585/680
  struct json_object *jso = json_tokener_parse("[{\"type\": \"needle\",\"kernelColorString\":\"-2 -2 2 0 0 0 0 0 0\",\
  \"boundingboxes\":[\
  {\"identifier\": \"water cons\", \"scaler\":-1,\"digit\":false, \"circle\": {\"cx\": 689, \"cy\": 449, \"cr\": 24}},\
  {\"identifier\": \"water cons\", \"scaler\":-2,\"digit\":false, \"circle\": {\"cx\": 661, \"cy\": 539, \"cr\": 24}},\
  {\"identifier\": \"water cons\", \"scaler\":-3,\"digit\":false, \"circle\": {\"cx\": 574, \"cy\": 576, \"cr\": 24}, \"confidence_id\": \"conf_circle\"},\
...
void MeterOCR_Test::test_calcImpulses()
{
	...
	// wrap detection:
	ASSERT_EQ(4, m.calcImpulses(0.2, 0.8)); // without wrap det. it would be negative (new value < old value)
	// multiple wraps:
...
TEST(MeterOCR, calcImpulses)
{
	MeterOCR_Test::test_calcImpulses();
}
...
TEST(MeterOCR, roundBasedOnSmallerDigits)
{
	MeterOCR_Test::test_roundBasedOnSmallerDigits();
}
