...
TEST(PushData, PDS_only_constructor)
{
...
TEST(PushData, PDS_constructor)
{
    ...
    // todo fix the comparision! 1.10000 vs. 1.10000001, ... ASSERT_EQ("{ \"data\": [ { \"uuid\": \"0\", \"tuples\": [ [ 1, 1.100000 ] ] } ] }", str);
}
...
TEST(PushData, PDS_fail_middleware)
{
    ...
    struct json_object *jso = json_tokener_parse("[{\"url\": \"http://127.0.0.1:45431/unit_test/push.json\"}]");
    ...
    pdl.add("0", 1, 1.1);
    pushDataList = &pdl;
    ASSERT_FALSE( pds.waitAndSendOnceToAll()); // we assume that localhost:45431/unit_test/push.json can't be connected to
    pushDataList = 0;
