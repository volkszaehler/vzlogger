...
void vz::api::InfluxDB::send()
{
	...
	_api.curl = curlSessionProvider ? curlSessionProvider->get_easy_session(_host + channel()->uuid()) : 0;
	...
	if(buf->size() > (unsigned) _max_buffer_size) {
		...
		unsigned int delta_delete = buf->size() - (unsigned) _max_buffer_size;  // number of items to delete from buffer
		buf->lock();
		it = buf->begin();
		while(!(delta_delete == 0 || (it == buf->end() ))) {  // buf->end() check shouldnt be necessary. But better safe than sorry.
			it->mark_delete();
			it++;
			delta_delete--;
		}
