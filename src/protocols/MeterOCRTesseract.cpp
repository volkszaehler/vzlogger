...
/*
	...
	TODO add offset/shift for needles (e.g. on the test pics the x0.1 needle is around a 18deg. late (e.g. at 5.4(from x0.01) the needle is at exactly 5 not at 5.4) (autolearn?)
	TODO needle autocalibrate (autofix like) in 2 pass approach? (1st pass as it is, 2nd pass with the center detection?)
	*/
	...
	_all_digits(true)
{
	...
	for (StdListBB::iterator it = _boxes.begin(); it != _boxes.end(); ++it){ // let's stick to begin not cbegin (c++11)
		...
		}else throw vz::OptionNotFoundException("boundingbox without box (circle?)");
	}
...
bool MeterOCR::RecognizerTesseract::recognize(PIX *imageO, int dX, int dY, ReadsMap &readings, const ReadsMap *old_readings, PIXA *debugPixa){
