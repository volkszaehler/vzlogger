...
const Option &OptionList::lookup(std::list<Option> const &options, const std::string &key) {
...
const char *OptionList::lookup_string_tolower(std::list<Option> const &options, const char *key) {
	std::string str = lookup_string(options, key);
	std::transform(str.begin(), str.end(), str.begin(), ::tolower);
	return str.c_str();
}

int OptionList::lookup_int(std::list<Option> const &options, const char *key) {
	const Option &opt = lookup(options, key);
	return (int)opt;
}

bool OptionList::lookup_bool(std::list<Option> const &options, const char *key) {
	const Option &opt = lookup(options, key);
	return (bool)opt;
}

double OptionList::lookup_double(std::list<Option> const &options, const char *key) {
	const Option &opt = lookup(options, key);
	return (double)opt;
}

struct json_object *OptionList::lookup_json_array(std::list<Option> const &options, const char *key) {
	const Option &opt = lookup(options, key);
	if (opt.type() != Option::type_array) throw vz::InvalidTypeException("json_object not an array");
	return (struct json_object *)opt;
}

struct json_object *OptionList::lookup_json_object(std::list<Option> const &options, const char *key) {
	const Option &opt = lookup(options, key);
